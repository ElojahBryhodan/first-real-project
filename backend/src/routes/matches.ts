...
const createMatchSchema = z.object({
  game: z.string().min(1),
  entryFeeCents: z.number().int().nonnegative().optional(),
});

matchesRouter.get('/', authMiddleware, async (req: AuthenticatedRequest, res, next) => {
  try {
...
matchesRouter.get('/:id', authMiddleware, async (req: AuthenticatedRequest, res, next) => {
  try {
...
matchesRouter.post('/', authMiddleware, async (req: AuthenticatedRequest, res, next) => {
  try {
...
matchesRouter.post('/:id/join', authMiddleware, async (req: AuthenticatedRequest, res, next) => {
  try {
    if (!req.user) return res.status(401).json({ error: 'Unauthorized' });
...
matchesRouter.post('/:id/finish', authMiddleware, async (req: AuthenticatedRequest, res, next) => {
  try {
    if (!req.user) return res.status(401).json({ error: 'Unauthorized' });
...
matchesRouter.post('/:id/dispute', authMiddleware, async (req: AuthenticatedRequest, res, next) => {
  try {
    if (!req.user) return res.status(401).json({ error: 'Unauthorized' });
    ...
    const isParticipant =
      match.createdById === userId || match.joinedById === userId;
